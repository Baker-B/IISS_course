...
const diseaseDescriptions = [
  ...
  {
    code: "copd",
    name: "Хронічне обструктивне захворювання легень",
  },
  {
    code: "commonCold",
    name: "Застуда",
  ...
  },
  {
    ...
    name: "Професійні захворювання легень",
  },
  {
    code: "pertussis",
    name: "Кашлюк",
...
document
  .querySelector('button[data-answer-type="text"]')
...
document
  .querySelector('button[data-answer-type="number"]')
  .addEventListener("click", (e) => {
    const age = parseInt(ageInputEl.value);
    if (Number.isInteger(age) && age > 0 && age < 150) {
      answers.age = age;
      if (answers.age <= 1) facts.ageGroup = "infants";
      else if (answers.age <= 13) facts.ageGroup = "children";
      else if (answers.age <= 35) facts.ageGroup = "youngAdults";
      else if (answers.age <= 55) facts.ageGroup = "middleAged";
      else facts.ageGroup = "elderly";
    ...
    } else document.getElementById("ageInputReminder").style.display = "block";
  });
...
const choiceBtns = Array.from(
  document.querySelectorAll('button[data-answer-type="choice"]')
);
choiceBtns.forEach((btn, i) => {
  btn.addEventListener("click", (e) => {
    ...
    const nextQuestion = document.querySelector(
      `div[data-number="${currentCardNumber + 1}"]`
    );
